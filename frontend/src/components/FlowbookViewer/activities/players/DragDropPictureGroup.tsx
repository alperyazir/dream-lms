...
interface DragDropPictureGroupProps {
...
export function DragDropPictureGroup({ activity }: DragDropPictureGroupProps) {
  ...
  const dropZones = (config.answer || []).map((zone, index) => ({
    id: `zone-${index}`,
    ...zone,
  }));
  ...
  const isCorrect = (zoneId: string): boolean | null => {
    ...
    if (!word || !zone?.group) return null;
    return zone.group.some(
      (groupWord) =>
        groupWord.toLowerCase().trim() === word.text.toLowerCase().trim(),
    );
  };

  const checkAllCorrect = useCallback(() => {
    if (Object.keys(placements).length !== dropZones.length) return false;

    return dropZones.every((zone) => {
      const wordId = placements[zone.id];
      if (!wordId) return false;
      const word = words.find((w) => w.id === wordId);
      if (!word || !zone.group) return false;
      return zone.group.some(
        (groupWord) =>
          groupWord.toLowerCase().trim() === word.text.toLowerCase().trim(),
      );
    ...
    (show: boolean) => {
      if (show) {
        ...
        dropZones.forEach((zone) => {
          if (zone.group && zone.group.length > 0) {
            const matchingWord = words.find(
              (w) =>
                !usedWords.has(w.id) &&
                zone.group.some(
                  (gw) =>
                    gw.toLowerCase().trim() === w.text.toLowerCase().trim(),
                ),
            );
            if (matchingWord) {
  ...
  const handleShowNextAnswer = useCallback(() => {
    ...
    for (const zone of dropZones) {
      if (!placements[zone.id] && zone.group) {
        const matchingWord = words.find(
          (w) =>
            !usedWords.has(w.id) &&
            zone.group.some(
              (gw) => gw.toLowerCase().trim() === w.text.toLowerCase().trim(),
            ),
        );
        if (matchingWord) {
          setPlacements((prev) => ({
              ...
              <div className="absolute inset-0 pointer-events-none">
                {dropZones.map((zone) => {
                  ...
                  if (isPercentage) {
                    ...
                    widthPercent =
                      (zone.coords.w / imageDimensions.width) * 100;
                    heightPercent =
...
function WordBank({
  ...
  const draggingWord = activeId
    ? allWords.find((w) => w.id === activeId)
    : null;
