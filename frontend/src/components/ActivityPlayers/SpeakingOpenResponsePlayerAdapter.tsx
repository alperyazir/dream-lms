...
export function SpeakingOpenResponsePlayerAdapter({
  ...
  // Load existing audio URL for current item
  useEffect(() => {
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        if (currentItem) {
          ...
          setAnswers((prev) => {
            const next = new Map(prev)
            next.set(currentItem.item_id, "recorded")
            return next
          })
        }
        // Stop all tracks
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-6 max-w-2xl mx-auto">
      {/* Context card */}
      {currentItem.context && (
        <div className="w-full px-4 py-3 rounded-lg bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800">
      ...
      {currentItem.grading_rubric && currentItem.grading_rubric.length > 0 && (
        <div className="w-full px-4 py-3 rounded-lg bg-muted/50 border border-muted-foreground/20">
          <p className="text-xs text-muted-foreground font-medium mb-1">
            Tips for a great response:
          </p>
          <ul className="text-xs text-muted-foreground space-y-0.5">
            {currentItem.grading_rubric.map((hint, i) => (
      ...
      {/* Mic error */}
      {micError && (
        <div className="w-full px-4 py-2 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="flex items-center gap-3">
            <button
              ...
              className="w-14 h-14 rounded-full bg-purple-500 hover:bg-purple-600 text-white flex items-center justify-center shadow-lg transition-all"
            >
              {isPlaying ? (
