...
export function VocabularyMatchingPlayerAdapter({
  ...
  const [matchOrder, setMatchOrder] = useState<string[]>(() => {
    if (initialAnswers && initialAnswers.size > 0)
      return Array.from(initialAnswers.keys());
    return [];
  });
  ...
  const handleDefinitionClick = useCallback(
    (defId: string) => {
      ...
      setMatchOrder((prev) => [
        ...prev.filter((id) => id !== selectedWord),
        selectedWord,
      ]);
      setSelectedWord(null);
    },
        ...
        <CardContent className="p-6">
          ...
          {showResults && showCorrectAnswers && (
            <div className="mt-6 rounded-lg border bg-muted/50 p-4">
              ...
              <div className="space-y-2">
                {words.map((w) => {
                  const correctDef = definitions.find(
                    (d) => d.def_id === w.pair_id,
                  );
                  return (
                    <div
                      key={w.pair_id}
