...
function DragDropReview({
  ...
  useEffect(() => {
    getActivityImageUrl(bookId, config.section_path)
      .then(setImgUrl)
      .catch(() => {});
  ...
  const zones = config.answer.map((ans) => {
    ...
    const studentText = rawVal
      ? resolveItemId(String(rawVal), config.words)
      : "";
  ...
  return (
    <div
      ref={containerRef}
      className="relative rounded-lg border bg-white dark:bg-neutral-900 overflow-auto max-h-[400px]"
    >
      <img
        ref={imageRef}
        src={imgUrl}
...
function CircleReview({
  ...
  if (isMultiSelect) {
    config.answer.forEach((a, i) => {
      if (a.isCorrect) correctSet.add(i);
    });
  } else {
    ...
    <div className="relative rounded-lg border bg-white dark:bg-neutral-900 overflow-auto max-h-[400px]">
      ...
      {config.answer.map((ans, idx) => {
        ...
        return (
          ...
          >
            <div
              className={`absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full text-xs font-bold shadow ${iconClass}`}
            >
              {iconText}
            </div>
          </div>
