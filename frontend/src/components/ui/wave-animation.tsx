...
export function WaveAnimation({ className = "" }: WaveAnimationProps) {
  ...
  const getWaveY = useCallback(
    (x: number, wave: Wave, time: number): number => {
      const dx = x * wave.length;
      const offsetY =
        Math.sin(dx + time * wave.speed + wave.phase) * wave.amplitude;
      const offsetY2 =
        Math.sin(dx * 0.5 + time * wave.speed * 0.8) * (wave.amplitude * 0.5);
  ...
  const updateAndDrawParticles = useCallback(
    (
      ...
      for (const particle of particlesRef.current) {
        ...
        if (particle.x > width) {
          particle.x = -10;
          particle.waveIndex = Math.floor(
            Math.random() * wavesRef.current.length,
          );
        }
        ...
        if (isDark) {
          ...
          gradient.addColorStop(
            0.5,
            `rgba(20, 184, 166, ${particle.opacity * 0.5})`,
          );
  ...
  return (
    <div
      className={`absolute bottom-0 left-0 right-0 h-[12vh] min-h-[80px] max-h-[200px] overflow-hidden ${className}`}
    >
