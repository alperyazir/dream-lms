...
function AssignmentSuccessScreen() {
  ...
  const MANUALLY_GRADED_TYPES = [
    "writing_free_response",
    "speaking_open_response",
  ];
  const isManuallyGraded = MANUALLY_GRADED_TYPES.includes(
    resultData?.activity_type || "",
  );
  ...
  const score = useMemo(() => {
    ...
    switch (activity_type) {
      case "ai_quiz":
      case "listening_quiz":
        parsedResult = parseAIQuizResult(
          config_json,
          answers_json,
          storedScore,
        );
      ...
      case "listening_word_builder":
        parsedResult = parseWordBuilderResult(
          config_json,
          answers_json,
          storedScore,
        );
      ...
      case "vocabulary_matching":
        ...
        break;
      case "mix_mode":
        parsedResult = parseMixModeResult(
          config_json,
          answers_json,
          storedScore,
        );
    ...
    if ("question_results" in parsedResult) {
      ...
      correct = parsedResult.item_results.filter(
        (r: any) => r.is_correct,
      ).length;
      total = parsedResult.total;
    }
