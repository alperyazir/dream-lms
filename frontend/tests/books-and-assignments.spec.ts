...
test.describe("Book Catalog (AC1)", () => {
  ...
  test("search filters books by title", async ({ page }) => {
    ...
    const bookTitles = await page
      .locator('[data-testid="book-card"] h3')
      .allTextContents()
  ...
  test("filter by publisher works", async ({ page }) => {
    ...
    await page
      .locator('select[name="publisher"]')
      .selectOption("Dream Publisher")

    // Wait for filtered results
    await page.waitForTimeout(500)

    // Verify publisher text appears in filtered books
    const publisherTexts = await page
      .locator('[data-testid="book-card"]')
      .locator('text="Dream Publisher"')
      .count()
  ...
  test("clicking 'View Activities' navigates to book detail", async ({
    page,
  }) => {
    ...
    await page
      .locator('[data-testid="book-card"]')
      .first()
      .getByRole("link", { name: /view activities/i })
      .click()
...
test.describe("Book Detail Page (AC2)", () => {
  ...
  test("displays activities table with 'Assign' buttons", async ({ page }) => {
    ...
    const assignButtons = await page
...
test.describe("Assignment Creation Wizard (AC3)", () => {
  ...
  test("completes full 4-step wizard flow", async ({ page }) => {
    ...
    await expect(
      page.locator('text="Assignment created successfully"'),
  ...
  test("validates required fields in Step 3", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/required/i").or(page.locator("text=/fill/i")),
    ).toBeVisible()
  })
...
test.describe("Teacher Assignment List (AC4)", () => {
  ...
  test("displays assignments table with required columns", async ({ page }) => {
    ...
    // Verify table headers
    await expect(
      page.getByRole("columnheader", { name: "Assignment Name" }),
    ).toBeVisible()
    ...
    await expect(
      page.getByRole("columnheader", { name: "Completion" }),
    ).toBeVisible()
  })
  ...
  test("clicking 'View Details' navigates to assignment detail", async ({
    page,
  }) => {
    // Wait for assignments to load
    await page.waitForSelector("table tbody tr", { timeout: 5000 })
  ...
  test("displays newly created assignment immediately (DATA-001 fix verification)", async ({
    page,
  }) => {
    ...
    await page
      .getByLabel("Due Date")
      .fill(`${tomorrow.toISOString().split("T")[0]}T23:59`)
...
test.describe("Assignment Detail (AC5)", () => {
  ...
  test("displays assignment information", async ({ page }) => {
    ...
    await expect(
      page.locator('text="Due Date"').or(page.locator('text="Due:"')),
    ).toBeVisible()
    await expect(
      page
  ...
  test("displays student completion table", async ({ page }) => {
    ...
    await expect(
      page.getByRole("columnheader", { name: /student/i }),
    ).toBeVisible()
    await expect(
...
test.describe("Student Assignment View (AC6)", () => {
  ...
  test("displays assignment tabs: To Do, Completed, Past Due", async ({
    page,
  }) => {
  ...
  test("displays assignment cards with status badges", async ({ page }) => {
    ...
    if ((await assignmentCards.count()) > 0) {
      ...
      await expect(
        assignmentCards
          .first()
          .locator("text=/not started|in progress|completed/i"),
      ).toBeVisible()
    }
  })
  ...
  test("switching tabs shows different assignments", async ({ page }) => {
    ...
    await page.waitForTimeout(500)
    const _completedCount = await page
      .locator('[data-testid="assignment-card"]')
      .count()
  ...
  test("assignment cards show countdown timer for active assignments", async ({
    page,
  }) => {
    // Wait for assignments to load
    await page.waitForTimeout(1000)
  ...
  test("'Start Assignment' button is present on assignment cards", async ({
    page,
  }) => {
    // Wait for assignments to load
    await page.waitForTimeout(1000)
