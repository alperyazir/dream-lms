...
class TestPasswordReset:
    """Tests for admin password reset functionality [AC: 1-5]"""

    def test_reset_password_success(
        self, client: TestClient, session: Session, admin_token: str, teacher_user: User
    ):
        """Test successful password reset for a teacher user [AC: 1, 3, 4]"""
        ...
        assert len(data["new_password"]) >= 12  # Password should be 12+ chars

        # Verify password was actually changed in database
        session.refresh(teacher_user)
        assert verify_password(data["new_password"], teacher_user.hashed_password)
        # Initial password should be cleared
        assert teacher_user.initial_password is None
    ...
    def test_reset_password_for_student(
        ...
        assert verify_password(data["new_password"], student_user.hashed_password)
    ...
    def test_reset_password_for_publisher(
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["email"] == publisher_user.email

        # Verify new password works
        session.refresh(publisher_user)
        assert verify_password(data["new_password"], publisher_user.hashed_password)
    ...
    def test_reset_password_requires_admin_role(
        ...
        """Test that non-admin users cannot reset passwords [AC: Admin only]"""
        response = client.post(
            f"{settings.API_V1_STR}/admin/users/{student_user.id}/reset-password",
    ...
    def test_reset_password_generates_secure_password(
        ...
        assert response.status_code == 200
        password = response.json()["new_password"]

        # Password should be at least 12 characters
        assert len(password) >= 12

        # Password should contain mixed characters (verify it's not just lowercase)
    ...
    def test_reset_password_creates_notification(
        ...
        """Test that password reset creates a notification for the user [AC: 6]"""
        # First get current notification count for user
        # (need teacher token to check their notifications)
        ...
        # Response structure is {"notifications": [...], "total": ..., ...}
        ...
        assert password_reset_notification is not None, "Password reset notification should exist"
