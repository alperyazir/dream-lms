...
def test_create_class_missing_required_fields(
    ...
    class_data = {"grade_level": "5th Grade", "subject": "Science"}
...
def test_get_class_detail_success(
    ...
    student = Student(id=uuid.uuid4(), user_id=student_user.id, grade_level="9th Grade")
    session.add(student)
    session.flush()
...
def test_add_students_to_class_success(
    ...
    class_obj = Class(
        id=uuid.uuid4(), name="Biology 101", teacher_id=teacher.id, school_id=school.id
    )
    session.add(class_obj)
    session.flush()
    ...
    student1 = Student(id=uuid.uuid4(), user_id=student1_user.id)
    ...
    student2 = Student(id=uuid.uuid4(), user_id=student2_user.id)
    session.add(student2)
    session.commit()

    # Test: Add students to class
    add_data = {"student_ids": [str(student1.id), str(student2.id)]}
...
def test_add_students_idempotent(
    ...
    class_obj = Class(
        id=uuid.uuid4(), name="Music 101", teacher_id=teacher.id, school_id=school.id
    )
    session.add(class_obj)
    session.flush()
    ...
    add_data = {"student_ids": [str(student.id)]}
...
def test_remove_student_from_class_success(
    ...
    remaining = session.exec(
        select(ClassStudent).where(
            ClassStudent.class_id == class_obj.id, ClassStudent.student_id == student.id
        )
    ).first()
...
def test_remove_student_not_enrolled(
    ...
    class_obj = Class(
        id=uuid.uuid4(), name="Drama 101", teacher_id=teacher.id, school_id=school.id
    )
    session.add(class_obj)
    session.flush()
