...
def upgrade():
    ...
    op.add_column(
        "book_assignments", sa.Column("dcs_book_id", sa.Integer(), nullable=True)
    )
    op.create_index(
        "ix_book_assignments_dcs_book_id", "book_assignments", ["dcs_book_id"]
    )
    ...
    op.drop_constraint(
        "book_assignments_book_id_fkey", "book_assignments", type_="foreignkey"
    )
