...
def upgrade():
    ...
    if not table_exists:
        ...
        bind.execute(
            sa.text(
                "CREATE INDEX ix_announcement_reads_student_id ON announcement_reads(student_id)"
            )
        )
        bind.execute(
            sa.text(
                "CREATE INDEX idx_student_read_at ON announcement_reads(student_id, read_at DESC)"
            )
        )
