...
def upgrade() -> None:
    ...
    op.create_index("ix_assignment_students_completed_at", "assignment_students", ["student_id", "completed_at"])
    # Also add missing indexes from medium findings
    op.create_index("ix_direct_messages_parent", "direct_messages", ["parent_message_id"])
    op.create_index("ix_generated_content_skill", "teacher_generated_content", ["skill_id"])
    op.create_index("ix_generated_content_format", "teacher_generated_content", ["format_id"])
