...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "report_type",
            sa.Enum("student", "class", "assignment", name="reporttypeenum"),
            nullable=False,
        ),
        sa.Column(
            "template_type", sqlmodel.sql.sqltypes.AutoString(length=100), nullable=True
        ),
    ...
    op.create_index(
        op.f("ix_report_jobs_teacher_id"), "report_jobs", ["teacher_id"], unique=False
    )
    op.create_index(
        op.f("ix_report_jobs_status"), "report_jobs", ["status"], unique=False
    )
    op.create_index(
        op.f("ix_report_jobs_expires_at"), "report_jobs", ["expires_at"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_saved_report_configs_teacher_id"),
        "saved_report_configs",
        ["teacher_id"],
        unique=False,
    )
...
def downgrade():
    ...
    op.drop_index(
        op.f("ix_saved_report_configs_teacher_id"), table_name="saved_report_configs"
    )
