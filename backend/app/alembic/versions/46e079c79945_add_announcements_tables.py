...
def upgrade():
    ...
    if not table_exists:
        ...
        bind.execute(
            sa.text(
                "CREATE INDEX ix_announcements_teacher_id ON announcements(teacher_id)"
            )
        )
        bind.execute(
            sa.text(
                "CREATE INDEX ix_announcements_created_at ON announcements(created_at DESC)"
            )
        )
        bind.execute(
            sa.text(
                "CREATE INDEX ix_announcements_deleted_at ON announcements(deleted_at)"
            )
        )
    ...
    if not table_exists:
        ...
        bind.execute(
            sa.text(
                "CREATE INDEX ix_announcement_recipients_announcement_id ON announcement_recipients(announcement_id)"
            )
        )
        bind.execute(
            sa.text(
                "CREATE INDEX ix_announcement_recipients_student_id ON announcement_recipients(student_id)"
            )
        )
        bind.execute(
            sa.text(
                "CREATE INDEX idx_student_created ON announcement_recipients(student_id, created_at DESC)"
            )
        )
