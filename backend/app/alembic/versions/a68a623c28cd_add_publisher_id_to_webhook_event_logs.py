...
def upgrade():
    ...
    op.add_column(
        "webhook_event_logs", sa.Column("publisher_id", sa.Integer(), nullable=True)
    )
    op.create_index(
        op.f("ix_webhook_event_logs_publisher_id"),
        "webhook_event_logs",
        ["publisher_id"],
        unique=False,
    )
...
def downgrade():
    ...
    op.drop_index(
        op.f("ix_webhook_event_logs_publisher_id"), table_name="webhook_event_logs"
    )
