...
def upgrade():
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "book_id", "publisher_id", name="uq_book_access_book_publisher"
        ),
    )
    op.create_index(
        op.f("ix_book_access_book_id"), "book_access", ["book_id"], unique=False
    )
    op.create_index(
        op.f("ix_book_access_publisher_id"),
        "book_access",
        ["publisher_id"],
        unique=False,
    )
    ...
    op.create_index(
        "idx_book_access_publisher_book",
        "book_access",
    ...
    op.add_column(
        "activities",
        sa.Column(
            "module_name", sqlmodel.sql.sqltypes.AutoString(length=255), nullable=False
        ),
    )
    op.add_column("activities", sa.Column("page_number", sa.Integer(), nullable=False))
    op.add_column(
        "activities", sa.Column("section_index", sa.Integer(), nullable=False)
    )
    op.drop_index("ix_activities_activity_type", table_name="activities")
    # Add indexes for activities
    op.create_index(
        "idx_activities_book_order",
        "activities",
        ["book_id", "order_index"],
        unique=False,
    )
    op.create_index(
        "idx_activities_book_type",
        "activities",
        ["book_id", "activity_type"],
        unique=False,
    )
    op.drop_index(
    ...
    op.add_column(
        "books",
        sa.Column(
            "book_name", sqlmodel.sql.sqltypes.AutoString(length=255), nullable=False
        ),
    )
    op.add_column(
        "books",
    ...
    op.add_column(
        ...
        sa.Column(
            "status",
            sa.Enum("published", "draft", "archived", name="bookstatus"),
            nullable=False,
        ),
    )
    ...
    op.create_index(
        op.f("ix_books_dream_storage_id"), "books", ["dream_storage_id"], unique=True
    )
    # Add composite index for books (publisher_id, status)
    op.create_index(
        "idx_books_publisher_status", "books", ["publisher_id", "status"], unique=False
    )
...
def downgrade():
    ...
    op.create_unique_constraint(
        "books_dream_storage_id_key", "books", ["dream_storage_id"]
    )
