...
def upgrade():
    ...
    if row and row[0] == "ARRAY":
        ...
        bind.execute(
            sa.text("ALTER TABLE feedback ALTER COLUMN emoji_reactions DROP DEFAULT")
        )
        bind.execute(
            sa.text(
                """
        ...
        bind.execute(
            sa.text("ALTER TABLE feedback ALTER COLUMN badges SET DEFAULT '[]'::jsonb")
        )
        bind.execute(
            sa.text(
                "ALTER TABLE feedback ALTER COLUMN emoji_reactions SET DEFAULT '[]'::jsonb"
            )
        )
...
def downgrade():
    ...
    if row and row[0] == "jsonb":
        ...
        bind.execute(
            sa.text("ALTER TABLE feedback ALTER COLUMN badges SET DEFAULT '{}'")
        )
        bind.execute(
            sa.text(
                "ALTER TABLE feedback ALTER COLUMN emoji_reactions SET DEFAULT '{}'"
            )
        )
