...
def upgrade():
    ...
    op.execute(
        "CREATE TYPE assignmentpublishstatus AS ENUM ('draft', 'scheduled', 'published', 'archived')"
    )

    # Add scheduled_publish_date column (nullable datetime)
    op.add_column(
        "assignments",
        sa.Column("scheduled_publish_date", sa.DateTime(timezone=True), nullable=True),
    )
    ...
    op.create_index(
        "ix_assignments_scheduled_publish_date",
        "assignments",
